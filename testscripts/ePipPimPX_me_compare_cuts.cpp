...
int cut_select;  //Select switch case (1, 2, 3, or 4)
float lcut_e_ele = 0.0;
float rcut_e_ele = 10.0;
float lcut_me = -0.40;
float rcut_me = 0.26;
float lcut_pim = -0.30;
float rcut_pim = 0.50;
...
for(cut_select = 1; cut_select < 5; cut_select++){

    bool cut1, cut2, cut3, cut4, cut_ele;
    ...
    */
    ...
    for(int j = 0; j < nentries; j++){
        ...
        //--------Define Cuts------------------------------
        cut_ele = e_ele > lcut_e_ele && e_ele < rcut_e_ele;
    ...
    c1->Print(Form("mm_me_cuts%i__ele_%.1f_%.1f.pdf", cut_select, lcut_e_ele, rcut_e_ele));
