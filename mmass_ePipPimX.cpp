...
int main(int argc, char **argv){
    ...
    TH1F *h1 = new TH1F("e#pi^{+}#pi^{-}", "missing mass", 200, 0.6, 1.4);
    ...
    for(int i = 1; i < argc; i++){
        ...
        t1->SetBranchAddress("prox", &prox);
        t1->SetBranchAddress("proy", &proy);
        t1->SetBranchAddress("proz", &proz);
        ...
        for(int j = 0; j < nentries; j++){
            ...
            double mass_ePipPimX = ePipPimX.M();

            h1->Fill(mass_ePipPimX);
        }
    }
